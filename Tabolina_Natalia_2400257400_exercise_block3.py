...
# read the data ----------------------
# -- pre coded --
pth = r'data_ex1ex3_waterquality.csv'
...
res_1 = []
for i in range(7):
  knn_i = KNeighborsClassifier(i+1)
  knn_i.fit(X_train, y_train)
  y_pred = knn_i.predict(X_test)
  y_proba = knn_i.predict_proba(X_test)[:, 1]
  res_1.append({"k": i+1, "accuracy": round(accuracy_score(y_test, y_pred), 3),
                "balanced_accuracy": round(balanced_accuracy_score(y_test, y_pred), 3),
                "roc_auc": round(roc_auc_score(y_test, y_proba), 3), "shuffle": False})
...
for i in range(7):
  ...
  y_pred = knn_i.predict(X_test)
  y_proba = knn_i.predict_proba(X_test)[:, 1]
  res_2.append({"k": i+1, "accuracy": round(accuracy_score(y_test, y_pred),
                3), "balanced_accuracy": round(balanced_accuracy_score(y_test, y_pred), 3),
                "roc_auc": round(roc_auc_score(y_test, y_proba), 3), "shuffle": True})

# build the dataframe
solution = pd.DataFrame(res_1 + res_2)
print('PART 1')
print(solution)
print()
...
# read the data ----------------------
# -- precoded --
pth = r'data_ex2_mosquitoIndicator.csv'
df = pd.read_csv(pth, sep=",")
...
# extract X,y arrays -----------------
# -- student work --
X = df.drop(columns=["mosquito_Indicator"])
...
for k in np.arange(1, 8, 1):
    ...
    collector.append({"k": k,
                      "mae": np.round(np.mean(collector_mae),3),
                      "rmse": np.round(np.mean(collector_rmse),3),
                      "medae": np.round(np.mean(collector_medae),3)})

# build DataFrame and return ----------
print('PART 2')
solution = pd.DataFrame(collector)
print(solution)
print()
...
collector = []
sh_split = StratifiedShuffleSplit(n_splits=5, test_size=0.2, random_state=42)
...
for k in np.arange(1,8,1):
    ...
    for train_idx, test_idx in sh_split.split(X_scale, y):
        X_train = X_scale[train_idx]
        y_train = y[train_idx]
        X_test = X_scale[test_idx]
        y_test = y[test_idx]
        ...
        collector_acc.append(np.round(accuracy_score(y_test, y_pred), 3))
        collector_bacc.append(np.round(balanced_accuracy_score(y_test, y_pred), 3))
        collector_bsc.append(np.round(brier_score_loss(y_test, y_hat_proba), 3))

    collector.append({"k": k,
                      "accuracy": np.round(np.mean(collector_acc), 3),
                      "balanced_accuracy": np.round(np.mean(collector_bacc), 3),
                      "brier_score": np.round(np.mean(collector_bsc), 3)})
